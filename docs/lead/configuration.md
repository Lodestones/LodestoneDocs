...
> [!ATTENTION]
> Before configuring Lead, it is recommended that you learn what [YAML](https://www.tutorialspoint.com/yaml/yaml_introduction.htm) is. It is a data serialization language.<br />
> Meaning that **ONE** invalid syntax will result in the plugin to fail.
...
> [!TIP|label:Hint]
> To reload teams without a restart (usually to import backups), you can add the `-t` flag after.

## config.yml

This is the default configuration of Lead as of `v1.1.4`.

> [!NOTE|label:Privating Commands]
> If you wish to change command's publicity, change the value from `null` to `"lodestone.lead.commands.[name]"`.<br/>
> You can use [LuckPerms](https://luckperms.net) for a permission overhaul.
